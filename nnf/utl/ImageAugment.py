...
class ImageAugment(object):
    ...
    ##########################################################################
    @staticmethod
    def gauss_data_gen(nndb, info=None, merge=False):
        ...
        info : :obj:`dict`, optional
            Provide additional information to perform gaussian data generation.
            (Default value = {}).
            ...
            - info['std'] = None                # Explicit definition of std ('noise_ratio' is ignored)
    ...
    def __ltransform(nndb, pp_params, im_per_class_multiples, save_folder=''):
        ...
        if (im_per_class_multiples != 0):
            ...
            for i in range(im_per_class_multiples):
                ...
                for (X_batch, X_batch) in ImageDataPreProcessor(pp_params, None).flow_ex(nndb, params):
                    if (cls_i >= nndb.cls_n):
                        break
